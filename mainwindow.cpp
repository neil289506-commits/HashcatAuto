...
void MainWindow::startHashcat() {
    if (!QFile::exists("hashcat.exe")) {
        appendLog("🌐 核心不存在，正在自動下載 Hashcat 6.2.6...", "#3498db");
        QNetworkReply *reply = navManager->get(QNetworkRequest(QUrl("https://hashcat.net/files/hashcat-6.2.6.7z")));
        connect(reply, &QNetworkReply::downloadProgress, this, &MainWindow::onDownloadProgress);
        connect(reply, &QNetworkReply::finished, [=](){ onDownloadFinished(reply); });
        downloadTimer.start();
        return;
    }
    ...
    // RTX 3050 加速優化: -w 3 (Workload Profile: High)
...
void MainWindow::onDownloadProgress(qint64 r, qint64 t) {
    if (t > 0) {
        double speed = (r / 1024.0 / 1024.0) / (downloadTimer.elapsed() / 1000.0);
        appendLog(QString("📥 下載進度: %1% | 速度: %2 MB/s").arg((double)r/t*100,0,'f',1).arg(speed,0,'f',2), "#f1c40f");
    }
}

void MainWindow::onDownloadFinished(QNetworkReply* reply) {
    if (reply->error() == QNetworkReply::NoError) {
        QFile f("hc.7z"); f.open(QIODevice::WriteOnly); f.write(reply->readAll()); f.close();
...
void MainWindow::clearResultFile() { 
    if(QFile::remove(outPathEdit->text())) appendLog("✅ 結果檔已清除", "#2ecc71"); 
}
